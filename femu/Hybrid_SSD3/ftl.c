...
static void ssd_init_params(struct Hybridparams *spp)
{
    ...
    spp->rb_blks_per_pl = 5; // read buffer的LUN中一个pl有4个block
    spp->slc_blks_per_pl = 34; // 一个pl中有88个SLC block
    spp->tlc_blks_per_pl = 100; // 一个pl中有676个TLC block
    ...
    spp->SLC_lru_space_len = 9449;
    spp->SLC_lru_capacity = 9449;
    spp->DRAM_lru_space_len = 944;
    spp->DRAM_lru_capacity = 944;
} // 混合SSD的基本架构参数
...
static void insert_dram_lru_page(struct ssd *ssd, uint64_t lpn, uint64_t rt)
{
    struct node_lru *lru = &ssd->DRAM_LRU;
    int hash_key = lpn % lru->capacity;
    int old_hash_key;
...
static uint64_t ssd_read(struct ssd *ssd, NvmeRequest *req, FILE *r_lat_log_file, FILE *hit)
{
    ...
    for (lpn = start_lpn; lpn <= end_lpn; lpn++) {
        ...
        if (dnode != NULL) {
            ...
            insert_dram_lru_page(ssd, lpn, req->stime);
            dnode->access_count++;
...
static void *ftl_thread(void *arg)
{
    ...
    FILE *workload_file1 = fopen("workload/PROJ_4.txt", "r");
    FILE *workload_file2 = fopen("workload/PROJ_4.txt", "r");
                ...
                if (req->nlb == 1) { // 初始化warm
                    ...
                    switch (req->cmd.opcode) {
                        ...
                        case NVME_CMD_READ: // 读操作
                            ...
                            if (ssd->sp.test_flag == true && ssd->sp.flag == 0) {
                                index++;
                                if (index == 1000) {
                                    ssd->sp.flag = 1;
                                }
