...
static void ssd_init_params(struct Hybridparams *spp)
{
    ...
    spp->rb_blks_per_pl = 2; // read buffer的LUN中一个pl有4个block
    ...
    if (ppa->g.lun < spp->rb_luns_per_ch) {
        ...
        switch (c) {
            case Hybrid_READ:
                ...
                //lat = (ch->next_ch2_avail_time < rb_lun->next_lun_avail_time) ? rb_lun->next_lun_avail_time : ch->next_ch2_avail_time;
...
static void class_predict(struct ssd *ssd, int index, int64_t time) {
    ...
    if ((class_end_lpn - class_start_lpn + 1) > 50){
