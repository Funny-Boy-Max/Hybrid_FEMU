...
struct Hybridparams {
    ...
    int nhistorys;    /* # of historys in the SSD for predict */
    int class_flag;   /* # Is the class ready */
    int max_nclass;   /* # Maximum address class for each sequence segment */
    int length_vq;    /* # Observation queue length */
    bool test_flag;
    ...
    uint64_t tlc_r_num;
    /*用于计算准确率*/
    uint64_t pre_count; // 记录预测的数量（以页为单位）
    uint64_t pre_hit_num; // 预测被访问计数（以页为单位）
...
typedef struct dram_node {
    ...
    bool begin_state; // 最刚开始加入LRU时，是预取进来的还是读取进来的【true:预取进来的 false: 读取进来的】
    bool pre_state; // 上一次访问的状态是读操作还是预取操作 【true:预取 false:读取】
    bool access_flag; // 与pre_state一直相反
    int access_count; // 记录dnode保存的数据在整个队列中是否被读取访问过
    int id;
...
struct address_class {
    uint64_t start_LBA;
    uint64_t end_LBA;
    int type;
    int height;
    struct address_class *left;
    struct address_class *right;
}; // Address Class Structure

struct class_space {
    int total_num;
    int used_num;
    struct address_class *head;
}; // Address Class Space

struct circulate_entry {
    uint64_t start_LBA;
    uint64_t end_LBA;    
};
...
struct ssd {
    ...
    struct history_entry *access_history; // 历史记录
    struct address_class *root; // Balanced binary search tree
    struct class_space *address_cs; // 地址类空闲空间，用于分配历史记录的划分类
    struct view_queue *vqueue; // 观测队列
    struct pre_queue *pqueue; // 预测队列
    struct mode_queue *mqueue; // 模式队列
